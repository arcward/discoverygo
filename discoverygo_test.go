...
func TestApiUrl(t *testing.T) {
	expectedUrl := "https://app.ticketmaster.com/discovery/v2"
	apiUrl, err := url.Parse(expectedUrl)
	if err != nil {
		t.Errorf("Unable to parse url: %v", err)
	}
	dc := DiscoveryClient{ApiUrl: *apiUrl}
	if dc.ApiUrl != *apiUrl {
		t.Errorf("Expected %v, got: %v", expectedUrl, dc.ApiUrl.String())
	}
}
...
func TestEventsUrl(t *testing.T) {
	...
	dc := DiscoveryClient{ApiUrl: *apiUrl}
	eventsUrl := dc.EventsUrl()
	expectedEventUrl := apiUrl.String() + "/events"
	if eventsUrl.String() != expectedEventUrl {
		t.Errorf(
			"Expected %v, got: %v",
			expectedEventUrl,
			eventsUrl.String(),
...
func TestAddApiKey(t *testing.T) {
	expectedUrl := "https://app.ticketmaster.com/discovery/v2"
	apiUrl, _ := url.Parse(expectedUrl)
	apiKey := "12345"
	dc := DiscoveryClient{ApiUrl: *apiUrl, ApiKey: apiKey}
	eventsUrlWithApiKey := dc.EventsUrl()
	expectedEventUrl := apiUrl.String() + "/events?apikey=" + apiKey

	if eventsUrlWithApiKey.String() != expectedEventUrl {
		t.Errorf(
			"Expected %v, got: %v",
			expectedEventUrl,
			eventsUrlWithApiKey.String(),
		)
	}
	fmt.Println(eventsUrlWithApiKey.String())
}
...
func TestGetEvent(t *testing.T) {
  ...
  "url": "http://ticketmaster.com/event/3B00506AA4EA161B"
	...
	decodeErr := json.Unmarshal([]byte(eventJson), &event)
	if decodeErr != nil {
		t.Errorf("Error decoding event json: %v", decodeErr)
	}
	fmt.Printf("%+v", event)
}
...
func TestUrlRedacted(t *testing.T) {
	...
	urlStr := redactUrl(dc.EventsUrl())
	if urlStr != fmt.Sprintf(
		"%s/events?apikey=%s",
		DiscoveryApiUrl,
		"REDACTED",
	) {
		t.Errorf(
			"Expected %v, got: %v",
			fmt.Sprintf("%s?apikey=%s", DiscoveryApiUrl, "1234"),
			urlStr,
		)
	}
}
